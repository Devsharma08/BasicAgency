...
const Projects = () => {
  ...
  return (
    <section className="w-full space-y-[7rem] py-20 overflow-x-hidden">
      <div
        ref={scrollRef}
        ...
        className={`flex flex-nowrap gap-2 pl-[4rem] pr-[32px] overflow-x-hidden select-none cursor-grab ${isDragging ? 'cursor-grabbing' : ''}`}
      >
        {resources.map((item, index) => (
          <div key={index} className='flex flex-col space-y-[1rem] group'>
            <div className="flex-shrink-0 w-[80vw] md:w-[450px] aspect-[4/5] bg-gray-200 overflow-hidden">
              {item.type === 'image' ? (
                <img src={item.link} alt={item.heading} className="w-full h-full object-cover pointer-events-none" />
              ) : (
      ...
      <div className='flex flex-col px-[4rem]'>
        ...
        <div className='flex font-[600] justify-between'>
          <p>
            <span className='leading-2 text-sm'>
              <span>00/</span>
